...
/*   Updated: 2022/06/25 18:41:17 by uercan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_window_size(t_list *game)
{
	...
	game->win = mlx_new_window(game->mlx, 32 * \
		game->max_x, 32 * game->max_y, "so_long");
}
...
static void	map_check_2(t_list *game)
{
...
void	ft_map_check(t_list *game, int x, int y)
{
	while (++y < game->max_y)
	{
		...
		while (++y < game->max_y)
		{
			...
			if (x != game->max_x && game->map[y][x + 1] != '\0')
				ft_exit(4);
		}
		map_check_2(game);
	}
